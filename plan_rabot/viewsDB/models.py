...
class Appeal(models.Model):
    ...
    EAM = models.ForeignKey(Detail, on_delete=models.CASCADE,
           max_length=100, verbose_name='№EAM')
    ...
    branch = (models.CharField
              (max_length=30, choices=BRANCH, default='accept', verbose_name='Филиал'))
    product_contact = (models.BooleanField
                       (verbose_name='Контакт с продуктом'))
    startAppeal_time = (models.DateField
                        (verbose_name='Время подачи заявки', null=True, blank=True))
    start_time = (models.DateField
                  (verbose_name='Время начала изготовления', null=True, blank=True))
    end_time = (models.DateField
                (verbose_name='Время окончания изготовления', null=True, blank=True))
    link = (models.CharField
            (max_length=150, verbose_name='Ссылка на заявку', null=True, blank=True, default='/'))
    machine = (MultiSelectField
               (max_length=110, choices=MACHINE, verbose_name='Станок', null=True, blank=True))
    quantity_defect = (models.IntegerField
                       (verbose_name='Брак', null=True, blank=True, default=0))
    ready_status = (MultiSelectField
                    (max_length=50, choices=READY_STATUS, default='accept', verbose_name='Ожидаем', null=True,
                     blank=True))
